...
<body class="<?php echo esc_attr(($is_wx||$is_h5)?'weixin-mode':'');?>">
...
<div class="content content-pay-wx">
    ...
    <div class="order-info-bar">
        <div class="order-number"><!--订单提交成功，请尽快付款!--> 购买：<?php echo esc_html($order_info->name);?></div>
        <div class="order-price">应付金额：<strong>&yen;<?php echo esc_html($order_info->money);?></strong>元</div>
    </div>

    <?php if($is_wx){?>
        <div class="wb-wxpay-btns">
            <a class="wb-btn wb-btn-primary" onclick="return callpay();">微信支付</a>
...
<?php }else if($is_h5){?>
    ...
    <div class="scan-qr-box">
        <div class="main">
            ...
            <div class="qrcode-box"><img src="<?php echo  WP_VK_URL;?>pay/wxpay/qrcode.php?data=<?php echo urlencode($code_url);?>" width="520" height="520" alt="二维码"></div>

            <?php if(wp_is_mobile() && !preg_match('#MicroMessenger#',$_SERVER['HTTP_USER_AGENT'])){?>
            <div class="qr-tips-wx">微信已不支持识别相册二维码<br>请在微信端打开网站或用另一个手机扫码</div>
            <?php }else{?>
            <div class="qr-tips">
        ...
        <div class="tips-pic"><img src="<?php echo esc_url(WP_VK_URL.'assets/img/wxp_pic.png'); ?>" alt="扫码提示"></div>
    </div>
...
<script type="text/javascript">
    ...
    function jsApiCall()
    {
        WeixinJSBridge.invoke(
            ...
            <?php echo $jsApiParameters?$jsApiParameters:"{}"; ?>,
            function(res){
                if(res.err_msg == 'get_brand_wcpay_request:ok'){
                    location.replace('<?php echo esc_url($post_url);?>');
                }else{
    ...
    (function($){
        var wxpay_check = function(){
            $.post('<?php echo esc_url(admin_url('/admin-ajax.php'));?>',{'action':'vk_pay','_ajax_nonce':'<?php echo esc_attr($_ajax_nonce);?>','op':'check','oid':'<?php echo absint($order_info->id);?>'},function(ret){
                if(ret.code){
                    setTimeout(function(){
                        wxpay_check()
                    },10000);
                }else if(!ret.code){
                    setTimeout(function(){location.replace('<?php echo esc_url($post_url);?>');},1000);
                }
            },'json');
        };
        setTimeout(function(){wxpay_check();},5000);
    })(jQuery);
