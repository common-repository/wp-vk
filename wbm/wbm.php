...
class WBMember
{
  ...
  public function render_menu()
  {
    ...
    foreach ($menus as $k => $m) {
      ...
      $html[] = '<li class="' . esc_attr($class_name) . '"><a href="' . esc_url($m['url']) . '"><svg class="wbm-icon wbmico-' .esc_attr( $k ) . '"><use xlink:href="#wbm-sico-' . esc_attr( $k). '"></use></svg><span>' . esc_html($m['name']) . '</span></a></li>';
    }
  ...
  {
    $wbp = sanitize_text_field($_GET['wbp'] ?? '');
    if (!$wbp) {
      return;
    }
    if ($wbp !== 'member') {
  ...
  public static function the_logo($echo = true)
  {
    ...
    $html .= '<div ';
    $html .= 'class="wbm-logo"><a href="' . esc_url(home_url('/')) . '" rel="home" title="' . esc_attr(get_bloginfo('name')) . '">';

    if ($logo_img) :
      $html .= '<img src="' . esc_attr($logo_img) . '" alt="' . esc_attr(get_bloginfo('name')) . '"/>';

    else :
      $html .= '<strong>' . esc_html(get_bloginfo('name')) . '</strong>';
    endif;
